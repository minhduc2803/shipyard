...
- **Integration toggles.** After `source "$HARNESS/bin/_common.sh"`, the pipeline checks which integrations are enabled with the shell function `harness_integration_enabled tracker|dev_qc|ci_wait` (exit 0 = on). **SKIP the stages for disabled integrations** — Stage 10 (Ticket) only runs if `harness_integration_enabled tracker`, Stage 12 (QC on dev) only if `harness_integration_enabled dev_qc`, and the deploy-wait inside dev-QC only if `harness_integration_enabled ci_wait`. (When all three are enabled, the full pipeline runs.)
...
source "$HARNESS/bin/_common.sh"           # reloads all config + integration toggles
setup_lane_env "$N"                         # re-exports API_BASE, FE_URL, DB vars, etc.
```
...
- **Integration toggles**: `harness_integration_enabled dev_qc && echo ON || echo OFF` (same for `tracker`, `ci_wait`) — do NOT skip or run integration stages from memory; always re-check.
...
- Open or update the PR targeting **main**: `gh pr create --base main --fill` (or `gh pr edit` if a prior run already created it). Capture the URL.
...
- **Only if `harness_integration_enabled tracker`** — otherwise skip this stage entirely (no ticket; leave `ticket_url` empty and note 'tracker integration off').
...
- **Only if `harness_integration_enabled dev_qc`** — otherwise skip (no dev-QC; set `qc_dev=` empty / note 'dev-QC off').
